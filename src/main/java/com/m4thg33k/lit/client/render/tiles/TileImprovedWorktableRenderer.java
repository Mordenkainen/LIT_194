...
public class TileImprovedWorktableRenderer extends TileEntitySpecialRenderer<TileImprovedWorktable> {

    private double factor = 0.1875f;

    public TileImprovedWorktableRenderer()
    {

    }

    @Override
    public void renderTileEntityAt(TileImprovedWorktable tile, double x, double y, double z, float partialTicks, int destroyStage) {
        if (tile==null)
        {
            return;
        }
        ...
        renderResult(tile.getWorld(),result,0,0.5,0,0.5);
    ...
    private void renderResult(World world, ItemStack stack, int xIndex, double extraHeight, int zIndex, double scale)
    {
        RenderItem itemRenderer = Minecraft.getMinecraft().getRenderItem();
        if (stack!=null)
        {
